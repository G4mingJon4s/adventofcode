...
void printPainted(Paint* p) {
        ...
        while (cur != NULL) {
            ...
            if (cur->value == 0) grid[y][x] = ' ';
            if (cur->value == 1) grid[y][x] = 'W';
            if (cur->value == 2) grid[y][x] = 'B';
            if (cur->value == 3) grid[y][x] = 'P';
            if (cur->value == 4) grid[y][x] = 'O';
...
void adjustJoystick(Paint* p, Linked* joystick) {
    Paint* cur = p;
    Paint* ball = NULL;
    while (cur != NULL) {
        if (cur->value == 4) {
    ...
    int diff = ball->point.x - stick->point.x;
    if (diff > 0) joystick->value = 1;
    if (diff == 0) joystick->value = 0;
    if (diff < 0) joystick->value = -1;
}

void printScreen(Paint* p, long long score, Linked* joystick) {
    printf("\033[2J");
    printf("\033[H");

    printf("Score: %10lld | Joystick: %+2lld\n", score, joystick->value);
    printPainted(p);
    adjustJoystick(p, joystick);
...
int main() {
    ...
    Linked joystick = { 0, NULL };
    printf("\033[?1049h");
    enableRawMode();

    while(1) {
        Linked* inputs = &joystick;
        int finished = 0;

        long long x = runIntcode(&mem, &inputs, &finished);
        if (finished) break;
        long long y = runIntcode(&mem, NULL, &finished);
        if (finished) break;
        long long p = runIntcode(&mem, NULL, &finished);
        if (finished) break;

        if (x == -1 && y == 0) {
            score = p;
            printScreen(painted, score, &joystick);
            continue;
        }

        Point tile = { x, y };
        Paint* cur = findPainted(painted, &tile);

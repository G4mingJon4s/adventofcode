...
int main() {
    ...
    int stepsTaken1 = 0;
    Step sw1;
    for (int s1 = 0; s1 < wire1->len; (s1++, x1 = x1n, y1 = y1n, stepsTaken1 += abs(sw1.dist))) {
        sw1 = wire1->steps[s1];
        x1n = x1;
        y1n = y1;
        if (sw1.dir == 'X') {
        ...
        int stepsTaken2 = 0;
        Step sw2;
        for (int s2 = 0; s2 < wire2->len; (s2++, x2 = x2n, y2 = y2n, stepsTaken2 += abs(sw2.dist))) {
            sw2 = wire2->steps[s2];
            x2n = x2;
            y2n = y2;
            ...
            if (sw1.dir == 'X') {
                ...
                if (((x1 < ix && ix < x1n) || (x1n < ix && ix < x1)) &&
                    ...
                    int sX = abs(ix - x1);
                    int sY = abs(iy - y2);
                    int sum = sX + sY + stepsTaken1 + stepsTaken2;
                    if (sum < minStepsTaken) minStepsTaken = sum;
                }
            } else {
                ix = x1;
                iy = y2;
